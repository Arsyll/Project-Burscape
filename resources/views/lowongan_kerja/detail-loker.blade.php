                                            ...
                                            <div class="col-lg-4 col-md-4 col-sm-4 d-flex flex-column">
                                                @if (!empty(auth()->user()))
                                                    @if (!empty(auth()->user()->user_role->alumni->lamaran))
                                                        @if (auth()->user()->user_role->alumni->lamaran->id_lowongan == $lowongan->id)
                                                        <button type="button" class="btn btn-secondary btn-lg">{{auth()->user()->user_role->alumni->lamaran->status}}</button>
                                                        <button class="btn btn-danger btn-lg mt-2" id="del-btn" href="#" data-id="{{auth()->user()->user_role->alumni->lamaran->id}}">
        ...
        <script>
            $(document).on('click', '#del-btn', function () {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Kamu yakin?',
                    text: "Kamu akan membatalkan lowongan {{auth()->user()->user_role->alumni->lamaran->lowongan->nama_lowongan ?? ''}}.",
                    type: 'error',
