    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-save-import').on('click', function() {
                var excel = $('#import_alumni').prop('files')[0];
                var fd = new FormData();
                if (excel != null) {
                    fd.append('import_alumni', excel);
                }
                fd.append('_token', '{{ csrf_token() }}');
                $.ajax({
                    type: "POST",
                    url: "{{ route('importAlumni') }}",
                    data: fd,
                    processData: false,
                    contentType: false,
                    enctype: "multipart/form-data",
                    success: function(response) {
                        ...
                        $(document).find('#form-modal-import').find('#btn-close-import')
                            .click();
                        getAlumni();
                        // if(response.code == 200){
                        //     return toastr.success(response.message, 'Success!', {
                    ...
                    error: function(reject) {
                        if (reject.status === 422) {
                            ...
                            var errors = message['errors'];
                            $.each(errors, function(key, val) {
                                $("#error_import_alumni").text(val[0]);
                            });
                            $('#btn-close-import').on('click', function() {
                                clearErrorMessages();
                    ...
                    success: function(response) {
                        ...
                        $(thisIs).parents(document).find('#btn-save-edit').on('click',
                            function() {
                                ...
                                var status = $('select[name=status] option').filter(
                                    ':selected').val();
                                var jurusan = $('select[name=id_jurusan] option').filter(
                                    ':selected').val();
                                var fd = new FormData();
                                if (foto != null) {
                                    fd.append('foto_profile', foto);
                                ...
                                if (conpassword != null) {
                                    fd.append('password_confirmation', conpassword);
                                }
                                fd.append('nama', name);
                                fd.append('no_telp', telp);
                                fd.append('tanggal_lahir', ttl);
            ...
            function clearErrorMessages() {
                ...
                $(document).find('#form-modal-import').find('#error_import_alumni').text("");
                            ...
                            columns: [{
                                ...
                                },
                                {
                                    ...
                                    render: function(data, type, row, meta) {
                                        refreshFsLightbox();
                                        return data ?
                                            '<a data-fslightbox="profile" href="{{ asset('storage/profile_alumni') }}/' +
                                            data + '">' +
                                            '<img src="{{ asset('storage/profile_alumni') }}/' +
                                            data +
                                            '" class="img-fluid bg-soft-info rounded" alt="profile-image">' +
                                            '</a>' :
                                            '<a data-fslightbox="profile" href="{{ asset('images/avatars/01.png') }}">' +
                                    ...
                                    render: function(data, type, row, meta) {
                                        return type === 'display' ?
                                            ...
                                            '<div class="col-4 me-1">'+
                                                '<a class="btn btn-primary btn-sm" id="show-btn" href="{{url('alumni/')}}/'+data+'">'+
                                            ...
                                            '</div>'+
                                            '<div class="col-4 me-1">' +
                                            '<button class="btn btn-warning btn-sm" id="update-btn" href="#" data-id="' +
                                            data +
                                            '" data-bs-toggle="modal" data-bs-target="#form-modal-edit">' +
                                            ...
                                            '</div>' +
                                            '<div class="col-4">' +
                                            '<button class="btn btn-danger btn-sm" id="del-btn" href="#" data-id="' +
                                            data + '">' +
...
<!-- Modal Import-->
<div class="modal fade" data-backdrop="static" id="form-modal-import" tabindex="-1" role="dialog"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <div class="modal-body">
                    <div class="container-fluid">
                        <div class="form-group">
                            <div><span class="text-danger" id="error_import_alumni"></span></div>
                            <label for="import_alumni">Import Excel</label>
                            <input type="file" class="form-control" id="import_alumni" placeholder="">
                        </div>
                        <a href="{{ route('download.template.alumni') }}" class="mt-1">Download Template</a>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" id="btn-close-import" class="btn btn-secondary"
                    data-bs-dismiss="modal">Close</button>
                <button type="button" id="btn-save-import" class="btn btn-primary">Import</button>
            </div>
        </div>
    </div>
</div>
